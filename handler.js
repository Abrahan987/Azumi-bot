...
let isSenderLid = m.sender.endsWith('@lid')
let senderJid

if (isSenderLid) {
    senderJid = global.lidJidCache.get(m.sender)
    if (!senderJid) {
        const participant = participants.find(p => p.id === m.sender)
        if (participant) {
            senderJid = participant.jid
            global.lidJidCache.set(m.sender, senderJid)
        } else {
            senderJid = m.sender
        }
    }
...
if (!user) {
  user = {
    jid: senderJid,
    lid: isSenderLid ? m.sender : undefined,
    name: m.name,
    exp: 0,
    coin: 0,
    bank: 0,
...
} else if (isSenderLid && user.lid !== m.sender) {
  user.lid = m.sender
}

m.sender = senderJid // Ensure m.sender is always a JID

let isAdmin = false, isSuperAdmin = false, isBotAdmin = false, isBotSuperAdmin = false
if (m.isGroup) {
  const participant = participants.find(p => this.decodeJid(p.id) === m.sender)
  if (participant) {
