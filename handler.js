...
let isSenderLid = originalSender.endsWith('@lid')
let senderJid

if (isSenderLid) {
    senderJid = global.lidJidCache.get(originalSender)
    if (!senderJid) {
        const participant = participants.find(p => p.id === originalSender)
        if (participant) {
            senderJid = participant.jid
            global.lidJidCache.set(originalSender, senderJid)
        } else {
            console.error(`Failed to resolve LID to JID for sender: ${originalSender}. Aborting message processing.`)
            return
        }
    }
} else {
    senderJid = originalSender
}

Object.defineProperty(m, 'sender', { value: senderJid, configurable: true })

let user = global.db.data.users[senderJid]
if (!user) {
  user = {
    jid: senderJid,
    lid: isSenderLid ? originalSender : undefined,
    name: m.name,
    exp: 0,
    coin: 0,
    bank: 0,
...
} else if (isSenderLid && user.lid !== originalSender) {
  user.lid = originalSender
}

let isAdmin = false, isSuperAdmin = false, isBotAdmin = false, isBotSuperAdmin = false
if (m.isGroup) {
  const participant = participants.find(p => p.id === originalSender || this.decodeJid(p.id) === m.sender)
  if (participant) {
    isAdmin = participant.admin === 'admin'
    isSuperAdmin = participant.admin === 'superadmin'
  }
  const botParticipant = participants.find(p => this.decodeJid(p.id) === this.decodeJid(this.user.jid))
  if (botParticipant) {
    isBotAdmin = botParticipant.admin === 'admin'
    isBotSuperAdmin = botParticipant.admin === 'superadmin'
  }
}
m.exp = 0
try {
