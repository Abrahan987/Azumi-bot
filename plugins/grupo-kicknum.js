const handler = async (m, {conn, args, groupMetadata, participants, usedPrefix, command, isBotAdmin, isSuperAdmin}) => {
  ...
  if (isNaN(args[0])) return conn.reply(m.chat, `❤️ Ingrese Algun Prefijo De Un Pais\nEjemplo: ${usedPrefix + command} 58`, m, rcanal);
  ...
  switch (command) {
    case 'listanum': case 'listnum':
      conn.reply(m.chat, `❤️ *Lista de numeros con el prefijo +${lol} que estan en este grupo:*\n\n` + numeros.join`\n`, m, {mentions: ps});
      break;
    case 'kicknum':
      if (!bot.restrict) return conn.reply(m.chat, '🥰 *¡Este Comando Esta Desabilitado Por El Propietario Del Bot!*', m, rcanal);
      if (!isBotAdmin) return m.reply('❤️ *El bot no es admin*');
